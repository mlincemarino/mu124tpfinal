...
```
#### IMP Arreglar CABA que sigue figurando como CIUDAD AUTONOMA
...
```{r}
# Calcular la línea de regresión en función de la variable 'diferencia'
linea_regresion_sel <- lm(diferencia_relativa ~ seleccionadas, data = mapa_mipieza_seleccionadas)

# Crear el gráfico ggplot con los cambios
graf_diferencia_sel <- ggplot(mapa_mipieza_seleccionadas, aes(x = seleccionadas, y = diferencia_relativa)) +
  geom_point_interactive(
    stroke = 1,
    alpha = 0.7,
    aes(
...
Ahora calculemos lo mismo asignando peso en el eje X a otra variable, que es la diferencia total (en puntos porcentales). 

```{r}
# Calcular la línea de regresión en función de la variable 'diferencia'
linea_regresion_sel_02 <- lm(diferencia_relativa ~ diferencia_final, data = mapa_mipieza_seleccionadas)

# Crear el gráfico ggplot con los cambios
graf_diferencia_sel <- ggplot(mapa_mipieza_seleccionadas, aes(x = diferencia_final, y = diferencia_relativa)) +
  geom_point_interactive(
    stroke = 1,
    alpha = 0.7,
    aes(
  ...
  geom_abline(intercept = coef(linea_regresion_sel_02)[1], slope = coef(linea_regresion_sel_02)[2], linetype = "dashed", color = "gray") +
  scale_x_continuous(
    name = "A DEFINIR",
    labels = scales::number_format()) +
  scale_y_continuous(
...
#### Sacar La Pampa porque genera un valor extremo que distorsiona. 
